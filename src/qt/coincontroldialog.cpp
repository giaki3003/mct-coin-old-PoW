...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = Bitcoinunits::MCTPP;
    ...
    QString toolTip4 = tr("Can vary +/- %1 mctposhi(s) per input.").arg(dFeeVary);
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            if(ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show mct address. In tree mode, address is not shown again for direct wallet address outputs
