...
#if defined(NDEBUG)
# error "MCT Core cannot be compiled without assertions."
#endif
...
/*
  int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
    ...
    if (halvings >= 35)
        ...
        else if (nHeight <= 259287 && nHeight > 210000) {
            return 25 * COIN;
        }
        else if (nHeight > 259287 && nHeight <= 348201) {
            return 8 * COIN;
        }
                         ...
                         CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // MCT : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, expectedReward, pindex->nMint, strError)) {
        return state.DoS(0, error("ConnectBlock(MCT): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(coinbaseTransaction, pindex->nHeight, expectedReward, pindex->nMint)) {
        ...
        return state.DoS(0, error("ConnectBlock(MCT): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // MCT : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(MCT): spork is off, skipping transaction locking checks\n");
    }
