...
/** 
 * Fees smaller than this (in mctoshis) are considered zero fee (for transaction creation)
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                   ...
                   if(chainActive.Height() >= 269000) {
                            found = pcoin->tx->vout[i].nValue == 15000000*COIN;
                    } else {
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(chainActive.Height() >= 269000) {
                        if(fMasternodeMode && wtx.tx->vout[i].nValue == 15000000*COIN) continue;
                } else {
                        if(fMasternodeMode && wtx.tx->vout[i].nValue == 15000*COIN) continue;
                }
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(chainActive.Height() >= 269000) {
         if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == 15000000*COIN) continue; //masternode input
        } else {
        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == 15000*COIN) continue; //masternode input
        }
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 MCT."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-mct-address
                        CScript scriptChange;
