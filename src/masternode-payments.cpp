...
*
*   Why is this needed?
*   - In MCT some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all MCT specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransactionRef& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f MCT\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
