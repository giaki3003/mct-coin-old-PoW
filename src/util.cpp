...
const char * const BITCOIN_CONF_FILENAME = "mct.conf";
const char * const BITCOIN_PID_FILENAME = "mctd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "mct" is a composite category enabling all MCT-related debug output
            if(ptrCategory->count(string("mct"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MCTCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MCTCore
    // Mac: ~/Library/Application Support/MCTCore
    // Unix: ~/.mctcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MCTCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MCTCore";
#else
    // Unix
    return pathRet / ".mctcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty mct.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override mct.conf
