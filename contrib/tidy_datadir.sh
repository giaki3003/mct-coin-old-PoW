...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete MCT database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no MCT datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old MCT datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected MCT 0.7 datadir."
    ;;
  3)
    echo "Detected MCT pre-0.8 datadir."
    ;;
  4)
    echo "Detected MCT 0.8 datadir."
    ;;
esac
